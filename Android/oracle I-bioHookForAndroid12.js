...
function dumpSuperFields(val1){
  ...
  function hookFingerAuthentication(){
    ...
    */
    ...
    let HidlAuthenticationClient = Java.use("com.android.server.biometrics.sensors.fingerprint.hidl.FingerprintAuthenticationClient");
    HidlAuthenticationClient["onAuthenticated"].implementation = function (identifier, authenticated, hardwareAuthToken) {

        send(`biometrics.sensors.fingerprint.hidl.FingerprintAuthenticationClient.onAuthenticated is called: identifier=${identifier}, authenticated=${authenticated}, token=${hardwareAuthToken}`);
        send(`Reflecting all the fields`);
